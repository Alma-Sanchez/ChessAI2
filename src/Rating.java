        ...
        for (int i=0;i<64;i++) {
            switch (BoardMouseListener.chess.pieces[i/8][i%8].pieceletter) {
                ...
                case "Q": counter+=queenBoard[i/8][i%8];
                    break;
                case "A": if (material>=1750) {counter+=kingMidBoard[i/8][i%8]; counter+=BoardMouseListener.posibleA(Chess.kingPositionC).length()*10;} else
                {counter+=kingEndBoard[i/8][i%8]; counter+=BoardMouseListener.posibleA(Chess.kingPositionC).length()*30;}
                    break;
            }
        }
        return counter;
